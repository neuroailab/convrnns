...
def _get_func_from_kwargs(function, **kwargs):
    ...
    if (function is None) or callable(function):
        ...
        try:
            f = getattr(convrnns.utils.cells, function)
        except:
            try:
                f = getattr(convrnns.utils.model_tool, function)
            except:
                try:
...
def graph_from_json(json_file_name):
    ...
    for json_node in json_nodes:
        ...
        attr["cell"] = convrnns.utils.cells.GenFuncCell
        attr["kwargs"] = {}
...
def init_nodes(G, input_nodes, batch_size=256, channel_op="concat", to_exclude=None):
    ...
    for node, attr in G.nodes(data=True):
        if node not in input_nodes:
            ...
            pred_shapes = [
                G.node[pred]["output_shape"]
                for pred in G.predecessors(node)
                if pred not in exclude_preds
            ]
            attr["kwargs"]["harbor_shape"] = harbor_policy(
                pred_shapes, attr["kwargs"]["harbor_shape"], channel_op=channel_op
            )
        ...
        for node, attr in G.nodes(data=True):  # Loop over nodes
            if t == 0:
                ...
                for pred in sorted(G.predecessors(node)):
                    ...
                    _inp = cell.input_init[0](
                        shape=output_shape, name=pred + "/standin", **cell.input_init[1]
                    )
                    inputs.append(_inp)
